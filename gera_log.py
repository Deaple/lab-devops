import os, random, string, uuid, copy
...
def random_data(data):
    ...
    # CHANGE_REPO_NAME = random.choice([gera_repo_name(),''])
    ...
    # CHANGE_GTW_ID = ''.join(random.choices(string.ascii_lowercase+string.digits,k=10))
...
for l in range(50):
    ...
    FILE_PATH="/ssd/fake_data/s3/"

    #p_dia = faker.date_time_between(start_date='-30y', end_date='now').strftime("%d")
    #p_mes = str(random.randint(1,12))
    #p_mes = faker.date_time_between(start_date='-30y', end_date='now').strftime("%m")
    #p_ano = str(random.randint(2019,2023))
    p_dia = str(random.randint(1,31)).rjust(2, "0")
    p_mes="06"
    p_ano="2023"
    p_hora = str(random.randint(0,23)).rjust(2, "0")
    #p_hora = fake.date_time_between(start_date='-30y', end_date='now').strftime("%H")
    FILE_NAME=str(random.getrandbits(128))+'.json'
    #path_patc = f'{FILE_PATH}ano={p_ano}/mes={p_mes}/dia={p_dia}/'
    ...
    with open(json_file_name, '+a') as file:
        file.write(logs)
    print(f"Arquivo # {l} armazenado com sucesso.")
